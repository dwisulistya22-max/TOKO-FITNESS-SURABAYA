...
const FeaturedProducts = ({ activeCategory, onCategoryChange, isAdmin }: FeaturedProductsProps) => {
  ...
  // ---------------------------------------------------------
  // MENGAMBIL DATA PRODUK DARI SANITY STUDIO
  // ---------------------------------------------------------
  const fetchProductsFromSanity = async () => {
    try {
      ...
      if (data?.result && data.result.length > 0) {
        ...
        // Gunakan localStorage / default jika Sanity kosong
      ...
      console.error('Gagal mengambil data Sanity, menggunakan data cadangan:', error);
      const savedProducts = localStorage.getItem('fitness_products');
      if (savedProducts && savedProducts !== "[]") {
        setProducts(JSON.parse(savedProducts));
      }
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              ...
              <div className="md:w-1/2 p-8 overflow-y-auto">
                ...
                <a 
                  href={`https://wa.me/${STORE_CONFIG.phone || '6281234567890'}?text=Halo ${STORE_CONFIG.name}, saya tertarik dengan produk *${selectedProduct.name}* seharga *${formatPrice(selectedProduct.price)}*. Apakah stok masih ada?`}
                  target="_blank"
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
          ...
          <div className="flex gap-3">
            <button 
              ...
              title="Perbarui Data dari Sanity Studio"
            >
              🔄 Refresh Data
            </button>
