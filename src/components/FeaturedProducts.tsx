...
// LINK DEFAULT TOKO SHOPEE ANDA (DIPASTIKAN LENGKAP HTTPS://)
const FALLBACK_SHOPEE = 'https://shopee.co.id/search?keyword=toko%20fitness%20surabaya';

// FUNGSI PINTAR MEMPERBAIKI FORMAT LINK SHOPEE
const fixShopeeUrl = (url: any) => {
  if (!url || typeof url !== 'string' || url.trim() === '') {
    return (STORE_CONFIG as any).shopee || FALLBACK_SHOPEE;
  }
...
const FeaturedProducts = ({ activeCategory = 'Semua', onCategoryChange }: any) => {
  ...
  const [globalShopee, setGlobalShopee] = useState<string>(fixShopeeUrl((STORE_CONFIG as any).shopee));
  ...
  const fetchProductsAndStore = async () => {
    ...
    for (const id of PROJECT_IDS) {
      try {
        ...
        if (foundShopee) {
          setGlobalShopee(fixShopeeUrl(foundShopee));
        }
        ...
        if (prodData?.result?.length) {
          setProducts(
            prodData.result.map((item: any) => ({
              ...
              shopeeUrl: fixShopeeUrl(item.shopeeUrl || item.shopee || foundShopee || globalShopee)
            }))
          );
          break;
        }
  ...
  return (
    <section id="products" className="py-20 bg-white">
        ...
        {selected && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col md:flex-row relative">
              <button type="button" onClick={() => setSelected(null)} className="absolute top-4 right-4 z-10 bg-white p-2 rounded-full shadow hover:bg-red-600 hover:text-white transition-all">
                <X size={22} />
              </button>
              ...
              <div className="md:w-1/2 p-8 overflow-y-auto">
                ...
                <div className="space-y-3">
                  <a href={`https://wa.me/${waNumber}?text=Halo%20Surabaya%20Fitness,%20saya%20tertarik%20dengan%20produk%20*${encodeURIComponent(selected.name)}*`} target="_blank" rel="noopener noreferrer" className="w-full bg-green-500 hover:bg-green-600 text-white py-3.5 rounded-xl font-bold flex items-center justify-center gap-2">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="text-center py-16 text-red-600 font-bold animate-pulse">Memuat produk...</div>
        ) : (
