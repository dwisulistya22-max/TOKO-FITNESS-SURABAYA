          ...
          setProducts(
            prodData.result.map((item: any) => {
              ...
              if (Array.isArray(item.galleryImages)) {
                item.galleryImages.forEach((img: string) => { if (img && !allImages.includes(img)) allImages.push(img); });
              }
    ...
    <section id="products" className="py-20 bg-white">
      <AnimatePresence>
        {selected && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white w-full max-w-5xl rounded-3xl overflow-hidden shadow-2xl max-h-[94vh] flex flex-col md:flex-row relative">
              
              <button type="button" onClick={() => setSelected(null)} className="absolute top-4 right-4 z-20 bg-white/90 p-2 rounded-full shadow-lg hover:bg-red-600 hover:text-white transition-all">
                <X size={22} />
              </button>
              ...
              <div className="md:w-1/2 bg-gray-900 p-4 flex flex-col relative min-h-[350px]">
                <div className="relative w-full h-full rounded-2xl overflow-hidden bg-black flex items-center justify-center">
                  <img src={selected.images[activeImgIndex]} alt={selected.name} className="w-full h-full object-contain transition-all" />
                ...
                {selected.images.length > 1 && (
                  <div className="flex gap-2 mt-3 overflow-x-auto pb-1 justify-center">
                    {selected.images.map((img: any, idx: number) => (
                      <img key={idx} src={img} onClick={() => setActiveImgIndex(idx)} className={`w-14 h-14 rounded-lg object-cover cursor-pointer border-2 ${activeImgIndex === idx ? 'border-red-500' : 'border-transparent opacity-50'}`} />
                    ))}
                  </div>
                )}
              </div>
              ...
              <div className="md:w-1/2 p-8 overflow-y-auto">
                <div className="text-xs font-bold text-red-600 uppercase tracking-widest mb-2">{selected.category}</div>
                ...
                <div className="mb-8 space-y-4">
                  <div>
                    <h4 className="font-bold flex items-center gap-2 mb-2 border-b pb-2 text-gray-900"><Info size={18} className="text-red-600" /> Deskripsi</h4>
                ...
                <div className="space-y-3">
                  <a href={`https://wa.me/${waNumber}?text=Halo, saya tertarik dengan produk *${encodeURIComponent(selected.name)}*`} target="_blank" rel="noopener noreferrer" className="w-full bg-green-500 hover:bg-green-600 text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 shadow-lg">
                    <ShoppingCart size={20} /> Pesan via WhatsApp
                  </a>

                  <a href={selected.shopeeUrl} target="_blank" rel="noopener noreferrer" className="w-full bg-[#EE4D2D] hover:bg-[#d73211] text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 shadow-lg">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-12">
          <div>
            <h2 className="text-3xl md:text-5xl font-black text-gray-900 mb-3 uppercase tracking-tighter italic">Produk Unggulan</h2>
            <p className="text-gray-500">Kualitas Gym Profesional kini hadir di rumah Anda.</p>
          </div>
          <div className="flex gap-2">
             <button onClick={fetchProductsAndStore} className="bg-gray-100 hover:bg-gray-200 px-5 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2">🔄 Refresh Data</button>
             <a href={globalShopee} target="_blank" rel="noopener noreferrer" className="bg-[#EE4D2D] text-white px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 shadow-md hover:bg-[#d73211]">🧡 Shopee Mall</a>
          </div>
        </div>
              ...
              <div key={p.id} className="bg-white rounded-3xl border border-gray-100 overflow-hidden hover:shadow-2xl transition-all flex flex-col justify-between group">
                <div>
                  <div className="relative aspect-square bg-gray-50 overflow-hidden">
                    ...
                    {p.tag && <span className="absolute top-4 left-4 bg-red-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter">{p.tag}</span>}
                    {p.images.length > 1 && (
                      <span className="absolute bottom-4 right-4 bg-black/60 text-white text-[10px] font-bold px-2.5 py-1 rounded-lg backdrop-blur-md">📷 {p.images.length} FOTO</span>
                    )}
                  </div>
                  <div className="p-6">
                    <div className="text-[10px] text-red-600 font-bold uppercase mb-1">{p.category}</div>
                    <h3 className="font-bold text-gray-900 mb-2 line-clamp-2 leading-tight h-10">{p.name}</h3>
                    <div className="flex items-center gap-1 text-yellow-400 mb-3">
                      {[...Array(5)].map((_, i) => <Star key={i} size={12} fill={i < 5 ? 'currentColor' : 'none'} />)}
                      <span className="text-gray-400 text-[10px] ml-1">(12+)</span>
                    </div>
                ...
                <div className="p-6 pt-0 space-y-2">
                  <button type="button" onClick={() => openDetail(p)} className="w-full bg-gray-900 hover:bg-red-600 text-white py-3 rounded-xl text-xs font-bold transition-all">Detail & Galeri</button>
                  <a href={p.shopeeUrl} target="_blank" rel="noopener noreferrer" className="w-full bg-[#EE4D2D]/10 text-[#EE4D2D] hover:bg-[#EE4D2D] hover:text-white py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-1 transition-all border border-[#EE4D2D]/20">🧡 Beli di Shopee</a>
                </div>
              </div>
            ))}
          </div>
