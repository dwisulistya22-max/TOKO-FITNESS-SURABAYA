...
// LINK TOKO SHOPEE UTAMA ANDA (Bisa diganti dengan link toko shopee Anda)
const SHOPEE_STORE_URL = 'https://shopee.co.id/search?keyword=toko%20fitness%20surabaya';
...
const FeaturedProducts = ({ activeCategory, onCategoryChange, isAdmin }: FeaturedProductsProps) => {
  ...
  // ---------------------------------------------------------
  // MENGAMBIL DATA PRODUK DARI SANITY STUDIO (+ LINK SHOPEE)
  // ---------------------------------------------------------
  const fetchProductsFromSanity = async () => {
    try {
      ...
      if (data?.result && data.result.length > 0) {
        const formattedSanityProducts = data.result.map((item: any, index: number) => ({
          id: item._id || index,
          ...
          image: item.image || 'https://images.unsplash.com/photo-1517836357463-d25dfeac3438?q=80&w=800&auto=format&fit=crop',
          description: item.description || '',
          specs: item.specs || '',
          tag: item.tag || '',
          shopeeUrl: item.shopeeUrl || `https://shopee.co.id/search?keyword=${encodeURIComponent(item.name || 'fitness')}`
        }));
        
        setProducts(formattedSanityProducts);
      } else {
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              ...
              <div className="md:w-1/2 p-8 overflow-y-auto">
                ...
                {/* DUA TOMBOL ORDER: WHATSAPP & SHOPEE */}
                <div className="mt-8 space-y-3">
                  <a 
                    href={`https://wa.me/${STORE_CONFIG.phone || '6281332345448'}?text=Halo Surabaya Fitness, saya tertarik dengan produk *${selectedProduct.name}* seharga *${formatPrice(selectedProduct.price)}*. Apakah stok masih ada?`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full bg-green-500 hover:bg-green-600 text-white py-3.5 px-4 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg transition-all text-sm"
                  >
                  ...
                  <a 
                    href={selectedProduct.shopeeUrl || SHOPEE_STORE_URL}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full bg-[#EE4D2D] hover:bg-[#d73f1d] text-white py-3.5 px-4 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg transition-all text-sm"
                  >
              ...
              >
                ...
                <div className="p-6 pt-0">
                  <div className="flex items-center justify-between border-t border-gray-100 pt-4 gap-2">
                    <span className="text-base font-bold text-red-600 truncate">{formatPrice(product.price)}</span>
                    
                    <div className="flex gap-1.5">
                      {/* Tombol Shopee Kecil di Card */}
                      <a
                        href={product.shopeeUrl || SHOPEE_STORE_URL}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="bg-[#EE4D2D] text-white p-2 rounded-lg hover:bg-[#d73f1d] transition-colors text-xs font-bold flex items-center justify-center"
                        title="Beli Langsung di Shopee"
                      >
                        🧡 Shopee
                      </a>
                      ...
                      <button 
                        onClick={() => setSelectedProduct(product)}
                        className="bg-gray-100 text-gray-900 px-3 py-2 rounded-lg hover:bg-red-600 hover:text-white transition-colors text-xs font-bold"
                      >
                        Detail
                      </button>
                    </div>
